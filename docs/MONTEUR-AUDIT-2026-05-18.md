...
**Modus:** Code-Only-Audit (MCP-Blocker: Sebastian-PWs für `barger`/`riedmann` unbekannt; `planung@…/[altes PW, rotiert, wertlos]` → "Benutzer nicht gefunden [B20-B]", `guenther/[altes PW, rotiert, wertlos]` → "Benutzername und Passwort eingeben"; Chrome-Instabilität)
...
6. **chef-Login PW:** `planung@ep-kolar.at/[altes PW, rotiert, wertlos]` wird abgelehnt mit "Benutzer nicht gefunden [B20-B]" — entweder Username vs E-Mail-Login-Bug oder PW falsch. `guenther` username existiert (L2526).
