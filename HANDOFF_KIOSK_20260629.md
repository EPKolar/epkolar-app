...
**Kiosk-URLs:** `?screen=planung` (WochenplanTafel) · `?screen=monteure` (MonteurTafel). Login `lager` / `[altes PW, rotiert, wertlos]` (Rolle lager_display).
...
**Manuell:** GoTrue-Logins für **aliti** (Ismael Aliti, w-id mqyxfca35x6i) + **lager** angelegt, Passwort `[altes PW, rotiert, wertlos]`.
