# -*- coding: utf-8 -*-
"""
v3.9.878 - !isMob && isMob: drei Bloecke, die am Handy nie gerendert wurden.
...
Kommentare korrekt ueberspringt, ergab fuer index.html (APP_VERSION
3.9.877-supabase, 29.215 Zeilen, Selbsttest der Endtiefe = 0/0/0):

    Z21264 Sp35   , !isMob&&React.createElement(React.Fragment, null
                  -> diese runde Klammer schliesst erst auf Z21435

Innerhalb dieser Spanne, also innerhalb von "nur wenn NICHT mobil", lagen:

    Z21269-21290  , spezFz.length>0&&isMob&&(()=>{ ... })()
                  Spezialfahrzeug-Streifen fuer den gewaehlten Tag.
                  Eigener Kommentar Z21267: "v3.9.505 Mobile (<600px)".
    Z21426-21434  , React.createElement('div', {gap:isMob?8:6, ...,
                                flexDirection:isMob?"column":"row"})
                  Eigener Kommentar: "v3.9.506 Mobile: Footer column-Layout".
                  darin Z21427-21430  isMob?<Excel/PDF full-width>:null
                  darin Z21433        "N BVH · ⚠️ Konflikte" / "✅"

Zum Vergleich das MOBIL-Fragment daneben, sauber geschlossen:
    Z21243 Sp34   , isMob&&React.createElement(React.Fragment, null
                  -> schliesst Z21262, also VOR dem Waechter.
(Zeilennummern gelten fuer den Stand der Messung; die Riegel unten arbeiten
mit Textmarken, nicht mit Zeilennummern.)
...
!isMob-Zweig danebensteht (Z21431/21432) - der Fehler war unsichtbar.
...
MOB_PICKER = "cellPick&&!((cellPick.days||[]).includes('_bem'))"
MOB_CARDS = "rows.filter(r=>!_isPaddingRow(r)).map((r,idx)=>{const _rc=m=>{"
...
def test_der_mobile_streifen_haengt_weiter_am_gewaehlten_tag(wp):
    """Gegen die billigste Art, diese Riegel gruen zu bekommen: den Block
    einfach loeschen. Er muss den Tag aus dem Mobil-Header weiterverwenden."""
    auf = wp.index(MOB_FZ)
    zu = wp.index("})()", auf)
    block = wp[auf:zu]
    assert "[selDay]" in block, (
        "Der mobile Fahrzeugstreifen liest nicht mehr den gewaehlten Tag "
        "(selDay) - dann zeigt er am Handy die falschen Buchungen."
    )
    assert "conflict:assigned.length>1" in block, (
        "Die Doppelbelegungs-Erkennung im mobilen Fahrzeugstreifen ist weg - "
        "genau sie ist der Grund, warum der Streifen am Handy gebraucht wird."
    )
