...
def test_dringend_filter_uses_priority_or_termin():
    text = INDEX.read_text(encoding='utf-8')
    # v3.8.70: dringendeAS in useMemo-Wrap. v3.9.23: source kann arbeitsscheine ODER _visibleAS (Monteur-Filter) sein.
    m = re.search(r'const dringendeAS\s*=\s*(?:_react\.useMemo\.call\(void 0,\s*\(\)\s*=>\s*)?(?:arbeitsscheine|_visibleAS)\.filter\(([\s\S]{0,800}?)\)\.length', text)
    assert m
    body = m.group(1)
